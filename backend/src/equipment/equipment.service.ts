...
export class EquipmentService {
  ...
  async generateReport(id: number, res: Response) {
    ...
    equipment.reservations.forEach((r) => {
      ...
      totalDiasReservados += Math.abs(fim.getTime() - inicio.getTime()) / (1000 * 60 * 60 * 24);
    });

    totalHorasReservados = totalDiasReservados * 24;

    const tempoMedioReservaHoras = 
      totalReservas > 0 ? (totalHorasReservados / totalReservas).toFixed(1) : '0';

    const tempoMedioReservaFormatado = formatarDuracao(Number(tempoMedioReservaHoras));
    
    const diasTotaisExistencia =
      Math.abs(agora.getTime() - new Date(equipment.createdAt).getTime()) / (1000 * 60 * 60 * 24);
    ...
    doc
      .fillColor('#ffffff').fontSize(22).font('Helvetica-Bold')
    ...
    const sc = statusColors[equipment.status] ?? { bg: '#f1f5f9', text: '#475569' };
    ...
    metricCards.forEach((card, i) => {
      ...
      doc.fillColor('#64748b').fontSize(8).font('Helvetica')
        .text(card.label.toUpperCase(), x + 10, cardY + 10, { width: cardW - 20, characterSpacing: 0.5 });
    ...
    if (equipment.room) {
      y = infoRow('Prédio / Andar',
        `${equipment.room.building ?? '-'} · Andar ${equipment.room.floor ?? '-'}`, y);
    ...
    if (!reservaAtual) {
      ...
      doc.fillColor('#0f172a').fontSize(10).font('Helvetica')
        .text(r.user, col1X, y + 22, { width: contentW * 0.32 });
 
      doc.fillColor('#64748b').fontSize(8).font('Helvetica')
        .text('INÍCIO', col2X, y + 12);
      doc.fillColor('#0f172a').fontSize(10).font('Helvetica')
        .text(formatarDataHora(new Date(r.startDate)), col2X, y + 22, { width: contentW * 0.3 });
 
      doc.fillColor('#64748b').fontSize(8).font('Helvetica')
        .text('FIM', col3X, y + 12);
      doc.fillColor('#0f172a').fontSize(10).font('Helvetica')
        .text(formatarDataHora(new Date(r.endDate)), col3X, y + 22, { width: contentW * 0.28 });
 
      if (atrasada) {
        const badgeW = 70;
        doc.roundedRect(margin + contentW - badgeW - 14, y + 38, badgeW, 16, 8).fill('#fee2e2');
        doc.fillColor('#991b1b').fontSize(8).font('Helvetica-Bold')
          .text('ATRASADA', margin + contentW - badgeW - 14, y + 42, { width: badgeW, align: 'center' });
      }
    ...
    const ultimasReservas = equipment.reservations
      ...
      .sort((a, b) => new Date(b.reservation.startDate).getTime() - new Date(a.reservation.startDate).getTime())
      .slice(0, 3);
    ...
    if (!ultimasReservas.length) {
      ...
      function drawReservaHeader(headerY: number): number {
        ...
        doc.text('INÍCIO',    margin + 148,      headerY + 7, { characterSpacing: 0.8 });
        doc.text('FIM',       margin + 248,      headerY + 7, { characterSpacing: 0.8 });
        doc.text('DEVOLUÇÃO', margin + 348,      headerY + 7, { characterSpacing: 0.8 });
        doc.text('STATUS',    margin + 428,      headerY + 7, { characterSpacing: 0.8 });
        return headerY + 22;
      }
      ...
      ultimasReservas.forEach((item, index) => {
        ...
        doc.rect(margin, y, contentW, rowH).fill(index % 2 === 0 ? '#ffffff' : '#f8fafc');
 
        doc.fillColor('#0f172a').fontSize(9).font('Helvetica');
        doc.text(r.user,                                        margin + 8,  y + 9, { width: 132, ellipsis: true });
        doc.text(formatarDataHora(new Date(r.startDate)), margin + 148, y + 9, { width: 92  });
        doc.text(formatarDataHora(new Date(r.endDate)),   margin + 248, y + 9, { width: 92  });
        doc.text(
          r.returnedAt ? new Date(r.returnedAt).toLocaleDateString('pt-BR') : '-',
          margin + 348, y + 9, { width: 72 },
        );
 
        const rbadgeW = 64;
        doc.roundedRect(margin + 428, y + 6, rbadgeW, 16, 8).fill(rsc.bg);
        doc.fillColor(rsc.text).fontSize(8).font('Helvetica-Bold')
          .text(rsc.label, margin + 428, y + 10, { width: rbadgeW, align: 'center' });
    ...
    const proximasReservas = equipment.reservations
      ...
      .sort((a, b) => new Date(a.reservation.startDate).getTime() - new Date(b.reservation.startDate).getTime())
      .slice(0, 3);
    ...
    if (!proximasReservas.length) {
      ...
      function drawProximaReservaHeader(headerY: number): number {
        ...
        doc.fillColor('#475569').fontSize(8).font('Helvetica-Bold');
        doc.text('USUÁRIO',   margin + 8,       headerY + 7, { characterSpacing: 0.8 });
        doc.text('INÍCIO',    margin + 140,      headerY + 7, { characterSpacing: 0.8 });
        doc.text('FIM',       margin + 280,      headerY + 7, { characterSpacing: 0.8 });
        doc.text('STATUS',    margin + 420,      headerY + 7, { characterSpacing: 0.8 });
        return headerY + 22;
      }
      ...
      proximasReservas.forEach((item, index) => {
        ...
        doc.fillColor('#0f172a').fontSize(9).font('Helvetica');
        doc.text(r.user,                                  margin + 8,   y + 9, { width: 152, ellipsis: true });
        doc.text(formatarDataHora(new Date(r.startDate)), margin + 140, y + 9, { width: 142 });
        doc.text(formatarDataHora(new Date(r.endDate)),   margin + 280, y + 9, { width: 142 });
 
        const rbadgeW = 64;
        doc.roundedRect(margin + 420, y + 6, rbadgeW, 16, 8).fill('#e0e7ff');
        doc.fillColor('#4338ca').fontSize(8).font('Helvetica-Bold')
          .text('Agendada', margin + 420, y + 10, { width: rbadgeW, align: 'center' });
